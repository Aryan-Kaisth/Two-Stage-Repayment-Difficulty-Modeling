...
class ModelEvaluation:
    ...
    def initiate_model_evaluation(self) -> ModelEvaluationArtifact:
        ...
        # 2. Load candidate model from MLflow
        model_uri = self.model_trainer_artifact.trained_model_path
        logger.info(f"Loading candidate model from: {model_uri}")
        model = mlflow.sklearn.load_model(model_uri)

        # 3. Evaluate challenger performance on holdout test set
        ...
        # 4. Log test metrics back to the active training run in MLflow
        try:
            ...
            logger.warning(f"Could not attach test metrics to MLflow run {model_uri}: {e}")

        # 5. Champion vs. Challenger Decision Logic
        ...
        # 6. Promotion to MLflow Model Registry
        ...
        # 7. Persist Evaluation Audit Report
        evaluation_report = {
